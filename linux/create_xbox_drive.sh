...
function copy_xbox_data() {
    ...
    # rsync -rlgoDzv --size-only /media/ubuntu/LABEL1/ /media/ubuntu/LABEL/
    ...
    # rsync -rlgoDzv --size-only ${MOUNT_ROOT}/LABEL1/ ${MOUNT_ROOT}/LABEL/
...
function copy_xbox_part() {
    ...
    rsync -rlgoDzv --size-only "${MOUNT_ROOT}/${LABEL}1/" "${MOUNT_ROOT}/${LABEL}/"
